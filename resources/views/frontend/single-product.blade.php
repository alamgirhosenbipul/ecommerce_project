						...
						<div class="product-info">
							...
							<div class="stock-container info-container m-t-10">
								<div class="row">
									...
									<div class="col-sm-9">
										<div class="stock-box">
											<span class="value">@if($product->product_qty == NULL) No product available @else In Stock @endif</span>
										</div>	
									</div>
								</div><!-- /.row -->	
							</div><!-- /.stock-container -->
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="price-box">
											...
											<span class="price-strike">${{ price_bn($product->selling_price) }}</span>
											@else
										...
										<div class="form-group">
											...
											<select class="form-control" id="size">
												...
												@foreach($size_bn as $size)
											     <option value="{{ $size }}">{{ $size }} ইঞ্চি</option>
											    @endforeach
												@else
												@foreach($size_en as $size)
											     <option value="{{ $size }}">{{ $size }} inchis</option>
											    @endforeach
												@endif
									...
									<div class="col-sm-2">
										<div class="cart-quantity">
											<div class="quant-input">
								                ...
								                <input type="text" class="form-control" value="1" id="qty" min="1">
			...
			<div class="products">
				<div class="product">		
						<div class="product-image">
							...
							@if(session()->get('language')=='bangla')
						@if($product->discount_price == NULL)
							<div class="tag sale"><span>নতুন</span></div> 
						@else 
							<div class="tag sale"><span>{{ price_bn(round($discount)) }}%</span></div> 
						...
						<div class="product-info text-left">
							@if(session()->get('language')=='bangla')
							<h3 class="name"><a href="{{ url('single/product/'.$product->id.'/'.$product->product_slug_bn) }}">{{ $product->product_name_bn }}</a></h3>
							@else
							<h3 class="name"><a href="{{ url('single/product/'.$product->id.'/'.$product->product_slug_en) }}">{{ $product->product_name_en }}</a></h3>
							@endif
							<div class="rating rateit-small"></div>
